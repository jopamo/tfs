...
fn same_filesystem(src: &Path, dst: &Path) -> Result<bool> {
    ...
    let dst_parent_meta = std::fs::metadata(dst_parent).context("failed to stat destination parent")?;
...
pub fn cp(src: &Path, dst: &Path, recursive: bool) -> Result<OpResult> {
    ...
    if metadata.is_file() {
        ...
        let bytes = fs_extra::dir::get_dir_content(src)
            .context("failed to read directory")?
            .files
            .iter()
            .map(|f| std::fs::metadata(f).map(|m| m.len()).unwrap_or(0))
            .sum();
        fs_extra::dir::copy(src, dst, &options).context("recursive copy failed")?;
        Ok(OpResult {
            bytes_copied: bytes,
            final_dst: dst.to_path_buf(),
            overwritten: false,
